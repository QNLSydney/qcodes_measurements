...
class RPGWrappedBase(ObjectProxy):
    ...
    def __wrap__(self, *args, **kwargs):
        if args or kwargs:
            raise TypeError(
                f"RPGWrappedBase.__wrap__ expects no arguments. Got args={args}, kwargs={kwargs}"
            )
        self._remote_functions = {}
        self._remote_function_options = {}
    ...
    def wrap(cls, instance, *args, **kwargs):
        ...
        base_inst.__dict__ = {**base_inst.__dict__, **instance.__dict__}
    ...
    def autowrap(inst):
        ...
        if isinstance(inst, ObjectProxy):
            if isinstance(inst, RPGWrappedBase):
                logger.debug(
                    "Object already wrapped. Has types: %r.", inst.__class__.__mro__
                )
                return inst
            ...
            # Try to match the remote type
            logger.debug("Extracted remote type: %s.", remote_type)
            if remote_type in RPGWrappedBase._subclass_types:
                return RPGWrappedBase._subclass_types[remote_type].wrap(inst)

            # Otherwise look to see if we have an extended type
            logger.debug("Fallback matching %s", inst._typeStr)
            typestr = re.match(
                r"^<[a-zA-Z_.]+\.([a-zA-Z_]+)(?:\(.*\))? (?:object )?at 0x[0-9A-Fa-f]+>$",
                str(inst._typeStr),
            )
            if typestr:
            ...
            logger.debug(
                "Object is not an ObjectProxy. Has types: %r.", inst.__class__.__mro__
            )
            ...
            for cls in self.__class__.__mro__:
                if name in cls.__dict__:
                    ...
                    if hasattr(v, "__get__"):
                        ...
                        logger.debug(
                            "Found attr %s locally in subclass %r, and is a property. Returns value: %r.",
                            name,
                            cls,
                            attr,
                        )
                        ...
                        attr = v
                        logger.debug(
                            "Found attr %s locally in subclass %r. Returns value: %r.",
                            name,
                            cls,
                            attr,
                        )
                ...
                if name in self._base_inst.__dict__:
                    ...
                    logger.debug(
                        "Found attr %s locally in base_inst (%r). Returns value: %r",
                        name,
                        self._base_inst,
                        attr,
                    )
